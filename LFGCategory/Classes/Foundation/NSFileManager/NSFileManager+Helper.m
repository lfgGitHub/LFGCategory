...
+ (NSArray *)listFilesInDirectoryAtPath:(NSString *)path deep:(BOOL)deep {
    ...
    if (deep) {
        // 深遍历
        NSArray *deepArr = [self.defaultManager subpathsOfDirectoryAtPath:path error:&error];
        if (!error) {
            listArr = deepArr;
        }else {
            listArr = nil;
        ...
        // 浅遍历
        NSArray *shallowArr = [self.defaultManager contentsOfDirectoryAtPath:path error:&error];
        if (!error) {
            listArr = shallowArr;
        }else {
            listArr = nil;
...
+ (BOOL)createDirectoryAtPath:(NSString *)path error:(NSError *__autoreleasing *)error {
    BOOL isSuccess = [self.defaultManager createDirectoryAtPath:path withIntermediateDirectories:YES attributes:nil error:error];
    return isSuccess;
}
...
+ (BOOL)createFileAtPath:(NSString *)path content:(NSObject *)content overwrite:(BOOL)overwrite error:(NSError *__autoreleasing *)error {
    ...
    // 创建文件
    BOOL isSuccess = [self.defaultManager createFileAtPath:path contents:nil attributes:nil];
    if (content) {
...
+ (BOOL)copyItemAtPath:(NSString *)path toPath:(NSString *)toPath overwrite:(BOOL)overwrite error:(NSError *__autoreleasing *)error {
    ...
    // 复制文件
    BOOL isSuccess = [self.defaultManager copyItemAtPath:path toPath:toPath error:error];
...
+ (BOOL)moveItemAtPath:(NSString *)path toPath:(NSString *)toPath overwrite:(BOOL)overwrite error:(NSError *__autoreleasing *)error {
    ...
    // 移动文件
    BOOL isSuccess = [self.defaultManager moveItemAtPath:path toPath:toPath error:error];
...
+ (NSNumber *)sizeOfDirectoryAtPath:(NSString *)path error:(NSError *__autoreleasing *)error {
    if ([self isDirectoryAtPath:path error:error]) {
        ...
        while (file = [contentsEnumurator nextObject]) {
            NSDictionary *fileAttributes = [self.defaultManager attributesOfItemAtPath:[path stringByAppendingPathComponent:file] error:nil];
